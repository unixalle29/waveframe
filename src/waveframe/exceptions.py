...
class InvalidContextValueTypeError(WaveFrameError):
    def __init__(self, key: str, expected_type: type[object]) -> None:
        super().__init__(
            f"WaveFrame context value {key!r} has invalid type; expected {expected_type.__name__}"
        )


class ContextValueNotFoundError(WaveFrameError):
    def __init__(self, key: str) -> None:
        super().__init__(f"WaveFrame context value {key!r} was not found")
...
class UnsupportedHandlerParameterTypeError(WaveFrameError):
    def __init__(self, parameter_name: str) -> None:
        super().__init__(
            ...
            "Supported annotations: bytes, WaveFrameContext, FrameSender."
        )
